...
public abstract class LfuCache<T> {
    ...
    public synchronized void put(Long id, T value) {
        if (cache.containsKey(id)) {
            ...
            entry.frequency++;
            log.info(
                    "Cache updated: Updated item with ID {} in cache (frequency: {})",
                    id,
                    entry.frequency
            );
        } else {
    ...
    public synchronized void remove(Long id) {
        if (cache.remove(id) != null) {
            log.info("Cache removed: Removed item with ID {} from cache", id);
        }
    }

    public synchronized void clear() {
        cache.clear();
        log.info("Cache cleared: All items removed from cache");
    }
}
